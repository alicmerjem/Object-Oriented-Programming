package org.example.database;
import java.sql.*;

public class Database4 {
    private static final String CONNECTION_STRING = "jdbc:mysql://db1.ibu.edu.ba:3306/shopdb";
    private static final String USERNAME = "shop_user";
    private static final String PASSWORD = "ibuSHOP2505";
    private Connection connection = null;

    public Database4() {
        try {
            connection = DriverManager.getConnection(CONNECTION_STRING, USERNAME, PASSWORD);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }

    public void getUsersByNameAndCity(String name, String city) throws SQLException {
        PreparedStatement statement = this.connection.prepareStatement("SELECT * FROM users WHERE name = ? AND city = ?");
        statement.setString(1, name);
        statement.setString(2, city);

        ResultSet rs = statement.executeQuery();

        while(rs.next()) {
            System.out.println("Name: " + rs.getString("name") + ", City: " + rs.getString("city"));
        }
    }
}

